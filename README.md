...
* **Interactive Floor Maps:** Color-coded tables indicate whether they are *Available (Green)*, *Occupied (Red)*, or *Reserved (Amber)*.
...
* **Granular Item Tracking:** Transition items individually or as entire tickets through processing phases (*Pending* ➔ *Preparing* ➔ *Ready* ➔ *Done*).
...
io.on("connection", (socket) => {
  ...
  socket.on(SOCKET_EVENTS.JOIN_TABLE, (tableId) => socket.join(`table:${tableId}`));
...
const checkCollision = (tableId, x, y, w, h, floorId) => {
  ...
  if (x < 0 || y < 0 || x + w > floor.gridWidth || y + h > floor.gridHeight) return true;
  // Overlap verification
  return tables.some(t => t.id !== tableId && x < t.x + t.width && x + w > t.x && y < t.y + t.height && y + h > t.y);
};
```
