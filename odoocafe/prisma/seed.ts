...
const PRECALCULATED_PASSWORD_HASH = "$2a$12$N9qo8uLOqp.R1p2bWn1xduf6t45c7d24s35e26g27h28i29j30k31";
...
async function main() {
  ...
  console.log("🧹 Cleaning up old orders, payments, customers, products, and categories...");
  ...
  // Retrieve or create tables to link orders
  let tables = await prisma.table.findMany();
  if (tables.length === 0) {
    console.log("🪑 No tables found. Creating a Floor and 10 base tables...");
  ...
  const productPrefixes = [
    "Classic", "Premium", "Signature", "Organic", "Spicy", "Crispy", "Loaded", "Roasted", "Double",
    "House", "Smoked", "Sweet", "Wild", "Rustic", "Chef's Special", "Creamy", "Cheesy", "Gourmet"
  ];
  
  const productSuffixes = [
    "Special", "Supreme", "Delight", "Platter", "Combo", "Extravaganza", "Bowl", "Infusion", "Crunch",
    "Frenzy", "Bliss", "Harvest", "Zest", "Sensational", "Medley", "Royal", "Elite", "Original"
  ];

  const categoryBaseItems: Record<string, string[]> = {
    "Signature Coffees": ["Espresso", "Cappuccino", "Latte", "Macchiato", "Flat White", "Mocha", "Americano", "Ristretto", "Cortado", "Affogato"],
    "Specialty Teas": ["Chai", "Green Tea", "Matcha", "Earl Grey", "Chamomile", "Oolong", "Peppermint", "Hibiscus", "Darjeeling", "Jasmine"],
    "Cold Brews & Iced": ["Cold Brew", "Iced Latte", "Nitro Brew", "Iced Mocha", "Frappe", "Affogato Shake", "Iced Americano", "Tonic Espresso"],
    "Gourmet Burgers": ["Veg Burger", "Cheese Burger", "Chicken Burger", "Paneer Burger", "Mushroom Burger", "BBQ Burger", "Spicy Zinger"],
    "Artisanal Pizzas": ["Margherita", "Pepperoni", "Veggie Delight", "Paneer Tikka Pizza", "BBQ Chicken Pizza", "Four Cheese", "Garden Fresh"],
    "Pasta Bowls": ["Penne Alfredo", "Spaghetti Bolognese", "Mac and Cheese", "Pesto Pasta", "Lasagna", "Arrabbiata", "Carbonara"],
    "Fresh Salads": ["Caesar Salad", "Greek Salad", "Garden Salad", "Quinoa Salad", "Fruit Salad", "Avocado Salad", "Caprese Salad"],
    "Breakfast Mains": ["Avocado Toast", "Pancake Stack", "French Toast", "Omelette", "Waffles", "Eggs Benedict", "Granola Bowl"],
    "Savory Sides": ["French Fries", "Garlic Bread", "Onion Rings", "Potato Wedges", "Cheese Balls", "Bruschetta", "Mozzarella Sticks"],
    "Cakes & Gateaux": ["Chocolate Brownie", "Red Velvet", "Cheesecake", "Tiramisu", "Black Forest", "Apple Pie", "Lemon Tart"],
    "Cookies & Muffins": ["Choco Chip Cookie", "Oatmeal Cookie", "Blueberry Muffin", "Chocolate Cupcake", "Banana Bread Slice", "Donut"],
    "Waffles & Pancakes": ["Belgian Waffle", "Maple Pancake", "Nutella Crepe", "Berry Waffle", "Choco Pancake", "Honey Crepe"],
    "Smoothies & Shakes": ["Mango Shake", "Strawberry Smoothie", "Chocolate Shake", "Berry Blast", "Vanilla Shake", "Oreo Crumble"],
    "Fresh Fruit Juices": ["Orange Juice", "Watermelon Juice", "Pineapple Juice", "Apple Juice", "Fresh Lime Soda", "Mint Mojito"],
    "Appetizers & Starters": ["Paneer Tikka", "Chicken Wings", "Spring Rolls", "Nachos Loaded", "Corn Fritters", "Fish Fingers"],
  };
  ...
  for (const cat of categories) {
    ...
    for (let j = 1; j <= 28; j++) {
      const prefix = productPrefixes[(j + cat.sortOrder) % productPrefixes.length];
      const base = bases[(j) % bases.length];
      const suffix = productSuffixes[(j * 2 + cat.sortOrder) % productSuffixes.length];
      
      const productName = `${prefix} ${base} ${suffix}`;
      const productPrice = randomPrice(60, 450);
      const isFood = ["Gourmet Burgers", "Artisanal Pizzas", "Pasta Bowls", "Fresh Salads", "Breakfast Mains", "Savory Sides", "Cakes & Gateaux", "Cookies & Muffins", "Waffles & Pancakes", "Appetizers & Starters"].includes(cat.name);
  ...
  const firstNames = [
    "Aarav", "Ishaan", "Ananya", "Dia", "Kabir", "Aditya", "Rohan", "Siddharth", "Neha", "Priya",
    "Rahul", "Amit", "Vikram", "Sneha", "Karan", "Arjun", "Riya", "Pooja", "Varun", "Abhishek",
    "John", "David", "Emma", "Olivia", "James", "Sophia", "Michael", "Charlotte", "Robert", "Emily",
    "Rajesh", "Sanjay", "Anil", "Sunita", "Anita", "Geeta", "Vijay", "Deepak", "Ravi", "Sandeep",
    "Preeti", "Kiran", "Meera", "Kavita", "Jyoti", "Harish", "Manish", "Gaurav", "Shalini", "Divya"
  ];
  
  const lastNames = [
    "Mehta", "Sharma", "Iyer", "Sen", "Roy", "Patel", "Gupta", "Verma", "Rao", "Nair",
    "Singh", "Kumar", "Joshi", "Mishra", "Pandey", "Trivedi", "Desai", "Shah", "Reddy", "Choudhury",
    "Smith", "Johnson", "Williams", "Brown", "Jones", "Miller", "Davis", "Garcia", "Rodriguez", "Wilson",
    "Bose", "Dutta", "Das", "Banerjee", "Chatterjee", "Mukherjee", "Sen", "Goon", "Mitra", "Sarkar"
  ];
  ...
  const orderStatuses = ["PAID", "PAID", "PAID", "PAID", "PAID", "SENT", "SENT", "DRAFT", "CANCELLED"]; // Weighted statuses
  const orderSources = ["CASHIER", "CASHIER", "CASHIER", "CUSTOMER", "CUSTOMER"]; // Weighted sources
  ...
  for (let i = 1; i <= 450; i++) {
    ...
    orderDate.setHours(Math.floor(Math.random() * 12) + 8, Math.floor(Math.random() * 60)); // Between 8 AM and 8 PM
    ...
    for (const prod of selectedProductsForOrder) {
      ...
      itemsData.push({
        ...
        lineTotal,
        productId: prod.id,
        kdsStatus: (status === "DRAFT" ? "PENDING" : status === "PAID" || status === "CANCELLED" ? "DONE" : pickRandom(["PENDING", "PREPARING", "READY", "DONE"])) as KdsStatus,
        createdAt: orderDate,
        updatedAt: orderDate,
      });
    }
    ...
    if (status === "PAID" && subtotal > 150 && Math.random() < 0.2) {
      discountTotal = Math.random() < 0.5 ? 50.00 : parseFloat((subtotal * 0.1).toFixed(2));
    }
    
    const grandTotal = Math.max(0, parseFloat((subtotal + taxTotal - discountTotal).toFixed(2)));
    ...
    const order = await prisma.order.create({
      data: {
        ...
        grandTotal,
        tableId: table.id,
        customerId: source === "CUSTOMER" ? customer.id : (Math.random() < 0.3 ? customer.id : null),
    ...
    if (status === "PAID") {
      ...
      await prisma.payment.create({
        data: {
          amount: grandTotal,
          transactionRef: pm.type !== "CASH" ? `TXN-${Math.random().toString(36).substring(2, 11).toUpperCase()}` : null,
